...
If files found, present them to the user: "检测到以下可发布内容，请选择要发布的文件：" with the files as options. If only `.md` files found (no `.html`), note: "建议先运行 md-to-html 转换格式".
...
## Step 2: Publish to WeChat Official Account
...
test -f .content-skills/wechat-publisher/EXTEND.md && echo "project"

# Then user-level (cross-platform: $HOME works on macOS/Linux/WSL)
test -f "$HOME/.content-skills/wechat-publisher/EXTEND.md" && echo "user"
```
...
4. **No upstream found**: Only in this case, fall back to asking user for input
...
| Multiple skills | Ask the user to choose                |
...
Install the md-to-html skill from the content-utilities plugin first.
...
```bash
# Check project-level
test -f .content-skills/.env && grep -q "WECHAT_APP_ID" .content-skills/.env && echo "project"

# Check user-level
test -f "$HOME/.content-skills/.env" && grep -q "WECHAT_APP_ID" "$HOME/.content-skills/.env" && echo "user"
```
...
Where to save?
A) Project-level: .content-skills/.env (this project only)
B) User-level: ~/.content-skills/.env (all projects)
```
...
| Markdown input                                                 | Title/content | Yes (via skill) | Yes (via skill) |
| Multiple images                                                | Yes (up to 9) | Yes (inline)  | Yes (inline)      |
| Themes                                                         | No            | Yes           | Yes               |
| Auto-generate metadata                                         | No            | Yes           | Yes               |
| Default cover fallback (`imgs/cover.png`)                      | No            | Yes           | No                |
| Comment control (`need_open_comment`, `only_fans_can_comment`) | No            | Yes           | No                |
