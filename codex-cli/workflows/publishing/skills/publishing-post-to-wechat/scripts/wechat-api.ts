...
function loadConfig(): WechatConfig {
  const cwdEnvPath = path.join(process.cwd(), ".content-skills", ".env");
  const homeEnvPath = path.join(os.homedir(), ".content-skills", ".env");
  ...
  if (!appId || !appSecret) {
    throw new Error(
      ...
      "Set via environment variables or in .content-skills/.env file."
    );
  }
