...
arguments:
  - name: input
    description: "Daily-brief path, .openspec.json, pipeline.openspec.json, or seed topic"
---

## Step 1: Upstream Artifact Detection (MANDATORY — before ANY other interaction)

**CRITICAL**: You MUST complete this step BEFORE any other interaction, BEFORE asking any questions about topics, and BEFORE presenting any options to the user. Do NOT skip this step.
...
If files found, ask the user: "检测到以下每日简报，请选择要用作素材的文件：" with the files as options (plus a "自定义话题" option for starting fresh).
...
## Step 2: Execute Topic Brainstorm
...
> **CONSTRAINT — Upstream Artifact Auto-Detection is MANDATORY**: Before asking the user ANY question about seed topics or materials, you MUST first scan for existing upstream artifacts. Only ask the user when NO upstream artifact is found.
...
4. Use the input's categories and items as the basis for Step 2.1 trend analysis

### Step 2.0: Pre-flight — Detect Upstream Artifacts
...
- If files found, ask the user: "检测到以下每日简报，请选择要用作素材的文件：" with options (plus "自定义话题")
- If nothing found, proceed to ask user for seed topic

**Only after Step 2.0 completes**, continue to Step 2.1.

### Step 2.1: Analyze Current Trends and Gaps
...
- What are the top trends in AI right now? (use news-search CLI + HN MCP + arXiv MCP)
  - **Platform sources** (via news-search CLI, 24h freshness enforced):
    ...
    - Twitter/X: `${TS_RUNNER} news-search/scripts/search.ts twitter "AI trending" 20`
    - Reddit: `${TS_RUNNER} news-search/scripts/search.ts reddit "artificial intelligence" 10`
    - YouTube: `${TS_RUNNER} news-search/scripts/search.ts youtube "AI news this week" 5`
...
### Step 2.2: Generate 20+ Topic Ideas
...
### Step 2.4: Rank and Filter to Top 10
...
### Step 2.5: Create Briefs for Top 3
...
- Minimum fields:
  ...
  - `next.command`: `tr-deep-research`
  - `next.input`: top-ranked topic or brainstorm file path

**Next step**: Suggest running `tr-deep-research` with one of the top-ranked topics.
