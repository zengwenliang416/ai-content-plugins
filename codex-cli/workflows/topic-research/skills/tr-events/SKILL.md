---
name: tr-events
description: "Build an AI event calendar by gathering conferences and launches, estimating likely events from cadences, and mapping content opportunities around each"
arguments:
  - name: input
    description: "Time period to cover, upstream .openspec.json, or pipeline.openspec.json"
---

## Step 1: Upstream Artifact Detection (MANDATORY — before ANY other interaction)

**CRITICAL**: You MUST complete this step BEFORE any other interaction and BEFORE asking the user for period details. Do NOT skip this step.
...
If files found, ask the user: "检测到以下活动规划素材，请选择要用于事件日历的输入：" with files as options.
...
- Twitter/X: `${TS_RUNNER} news-search/scripts/search.ts twitter "AI conference 2026" 20`
- GitHub: `${TS_RUNNER} news-search/scripts/search.ts github "awesome-ai-conferences" 5`
- Web: `${TS_RUNNER} news-search/scripts/search.ts web "AI events calendar 2026" 10`
- Read event pages: `${TS_RUNNER} news-search/scripts/read.ts <event-url>`

### Step 2.2: Estimate Likely Events
...
### Step 2.3: Map Content Opportunities
...
**Output**: Event calendar saved to `openspec/runtime/events/YYYY-MM-DD-<period>-event-calendar.md`
...
- Minimum fields:
  ...
  - `next.command`: `content-plan`
  - `next.input`: event calendar path or contract path

**Next step**: Suggest running a content-plan skill to fold event windows into the editorial schedule.
