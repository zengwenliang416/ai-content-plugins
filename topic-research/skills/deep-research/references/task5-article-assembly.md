...
## Visual Embedding Protocol

### Step 1: Load Image Manifest

Read `<output-dir>/images/chart_index.txt` from Task 4.

- Verify all image files exist at the listed paths
- If any files are missing, check `~/banana-images/` and copy them over to `<output-dir>/images/`

### Step 2: Embed Images Using Relative Paths

Use this exact markdown format for every image in the article:

```markdown
![<Alt text description>](./images/<filename>.png)
_Figure N: <Caption>. Source: <Source>._
```

**Path rules**:

- ALWAYS use relative paths starting with `./images/`
- NEVER use absolute paths (e.g., `/Users/.../banana-images/...`)
- NEVER use bare filenames (e.g., `gen_*.png` without `./images/` prefix)
- The article .md file and the `images/` directory are siblings in the same output directory
- Use the `ARTICLE EMBEDDING` section from chart_index.txt as copy-paste reference

### Step 3: Verify After Writing

After writing the final article, verify:

- [ ] Every `![...](./images/...)` path points to an existing file
- [ ] No image references use bare filenames
- [ ] Figure numbers are sequential
- [ ] Every image has a caption with source attribution
