...
Env file load order: CLI args > EXTEND.md > process.env > <cwd>/.content-skills/.env > ~/.content-skills/.env`);
}
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--promptfiles") {
      ...
      if (items.length === 0)
        throw new Error("Missing files for --promptfiles");
    ...
    if (a === "--quality") {
      const v = argv[++i];
      if (v !== "normal" && v !== "2k")
        throw new Error(`Invalid quality: ${v}`);
      out.quality = v;
      continue;
    }
    ...
    if (a === "--imageSize") {
      ...
      if (v !== "1K" && v !== "2K" && v !== "4K")
        throw new Error(`Invalid imageSize: ${v}`);
      out.imageSize = v;
      continue;
    }
...
async function loadEnv(): Promise<void> {
  ...
  const homeEnv = await loadEnvFile(path.join(home, ".content-skills", ".env"));
  const cwdEnv = await loadEnvFile(path.join(cwd, ".content-skills", ".env"));
    ...
    if (trimmed.includes(":") && !trimmed.startsWith("-")) {
      ...
      if (key === "version") {
        ...
        config.default_quality =
          value === "null" ? null : (value as "normal" | "2k");
        ...
        config.default_image_size =
          value === "null" ? null : (value as "1K" | "2K" | "4K");
...
async function loadExtendConfig(): Promise<Partial<ExtendConfig>> {
  ...
  const paths = [
    path.join(cwd, ".content-skills", "ai-image-gen", "EXTEND.md"),
    path.join(home, ".content-skills", "ai-image-gen", "EXTEND.md"),
  ];
...
function detectProvider(args: CliArgs): Provider {
  ...
  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, DASHSCOPE_API_KEY, or REPLICATE_API_TOKEN.\n" +
      "Create ~/.content-skills/.env or <cwd>/.content-skills/.env with your keys.",
  );
}
...
async function main(): Promise<void> {
  ...
  if (!model && extendConfig.default_model) {
    if (provider === "google")
      model = extendConfig.default_model.google ?? null;
    if (provider === "openai")
      model = extendConfig.default_model.openai ?? null;
    if (provider === "dashscope")
      model = extendConfig.default_model.dashscope ?? null;
    if (provider === "replicate")
      model = extendConfig.default_model.replicate ?? null;
  }
