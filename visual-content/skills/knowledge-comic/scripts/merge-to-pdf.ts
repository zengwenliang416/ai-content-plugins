...
function parseArgs(): { dir: string; output?: string } {
  ...
  if (!dir) {
    console.error(
      "Usage: bun merge-to-pdf.ts <comic-dir> [--output filename.pdf]",
    );
    process.exit(1);
  }
...
async function createPdf(pages: PageInfo[], outputPath: string) {
  ...
  pdfDoc.setAuthor("knowledge-comic");
  ...
  for (const page of pages) {
    ...
    console.log(
      `Added: ${page.filename}${page.promptPath ? " (prompt available)" : ""}`,
    );
  }
...
async function main() {
  ...
  const dirName =
    basename(dir) === "comic" ? basename(join(dir, "..")) : basename(dir);
