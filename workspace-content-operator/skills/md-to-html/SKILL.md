...
test -f .content-skills/md-to-html/EXTEND.md && echo "project"

# Then user-level (cross-platform: $HOME works on macOS/Linux/WSL)
test -f "$HOME/.content-skills/md-to-html/EXTEND.md" && echo "user"
```
...
**If user agrees**: Invoke `md-formatter` skill to format the file, then use formatted file as input.
...
```bash
# Check wechat-publisher EXTEND.md for default_theme
test -f "$HOME/.content-skills/wechat-publisher/EXTEND.md" && grep -o 'default_theme:.*' "$HOME/.content-skills/wechat-publisher/EXTEND.md"
```
...
```bash
npx -y bun {baseDir}/scripts/main.ts <markdown_file> --theme <theme>
```
...
```bash
npx -y bun {baseDir}/scripts/main.ts <markdown_file> [options]
```
...
npx -y bun {baseDir}/scripts/main.ts article.md

# With specific theme
npx -y bun {baseDir}/scripts/main.ts article.md --theme grace

# Keep the first heading in content
npx -y bun {baseDir}/scripts/main.ts article.md --keep-title

# Override title
npx -y bun {baseDir}/scripts/main.ts article.md --title "My Article"
```
