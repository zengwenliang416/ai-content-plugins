...
**Agent Execution**: Use `{baseDir}` as this skill's directory, then use `{baseDir}/scripts/<name>.ts`.
...
```bash
npx -y bun {baseDir}/scripts/check-permissions.ts
```
...
```bash
npx -y bun {baseDir}/scripts/wechat-browser.ts --markdown article.md --images ./images/
npx -y bun {baseDir}/scripts/wechat-browser.ts --title "标题" --content "内容" --image img.png --submit
```
...
```bash
npx -y bun {baseDir}/scripts/wechat-api.ts <html_file> [--title <title>] [--summary <summary>] [--author <author>] [--cover <cover_path>]
```
...
```bash
npx -y bun {baseDir}/scripts/wechat-article.ts --html <html_file>
```
