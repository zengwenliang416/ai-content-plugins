...
3. `{baseDir}` resolves to this skill's directory at runtime
...
```bash
npx -y bun {baseDir}/scripts/check-paste-permissions.ts
```
...
```bash
npx -y bun {baseDir}/scripts/x-browser.ts "Hello!" --image ./photo.png
```
...
```bash
npx -y bun {baseDir}/scripts/x-video.ts "Check this out!" --video ./clip.mp4
```
...
```bash
npx -y bun {baseDir}/scripts/x-quote.ts https://x.com/user/status/123 "Great insight!"
```
