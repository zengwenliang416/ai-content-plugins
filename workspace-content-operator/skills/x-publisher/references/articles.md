...
npx -y bun {baseDir}/scripts/x-article.ts article.md

# With custom cover image
npx -y bun {baseDir}/scripts/x-article.ts article.md --cover ./cover.jpg

# Actually publish
npx -y bun {baseDir}/scripts/x-article.ts article.md --submit
```
...
npx -y bun {baseDir}/scripts/md-to-html.ts article.md

# Output HTML only
npx -y bun {baseDir}/scripts/md-to-html.ts article.md --html-only

# Save HTML to file
npx -y bun {baseDir}/scripts/md-to-html.ts article.md --save-html /tmp/article.html
```
...
- **Content not pasting**: Check HTML clipboard: `npx -y bun {baseDir}/scripts/copy-to-clipboard.ts html --file /tmp/test.html`
