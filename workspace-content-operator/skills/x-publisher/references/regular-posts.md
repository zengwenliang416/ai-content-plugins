...
```bash
npx -y bun {baseDir}/scripts/copy-to-clipboard.ts image /path/to/image.png
```
...
npx -y bun {baseDir}/scripts/paste-from-clipboard.ts

# Paste to Chrome with retries
npx -y bun {baseDir}/scripts/paste-from-clipboard.ts --app "Google Chrome" --retries 5

# Quick paste with shorter delay
npx -y bun {baseDir}/scripts/paste-from-clipboard.ts --delay 200
```
...
Claude:
1. Runs: npx -y bun {baseDir}/scripts/x-browser.ts "Hello from Claude!" --image ./screenshot.png
...
- **Image paste fails**:
  - Verify clipboard script: `npx -y bun {baseDir}/scripts/copy-to-clipboard.ts image <path>`
